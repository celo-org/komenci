...
describe("WalletService", () => {
  const deployerAddress = randomHex(20)
  ...
  describe('#deployWallet', () => {
    describe('with LegacyProxyDeployer', () => {
      const validImplementation = randomHex(20)
      const invalidImplementation = randomHex(20)
      const externalAccount = randomHex(20)
      ...
      beforeEach(async () => {
        module = await buildModule({
          ...
          contracts: {
            ProxyCloneFactory: randomHex(20),
            MetaTransactionWalletVersions: {
              [validImplementation]: "1.1.0.0"
            },
            MetaTransactionWalletDeployer: randomHex(20)
          }
        })
      })
        ...
        describe('on the first run', () => {
          it('sends a transaction and updates the session', async () => {
            ...
            if (deployRes.ok === true) {
              expect(deployRes.result).toMatchObject({ txHash })
    ...
    describe('with EIP1167ProxyDeployer', () => {
      const validImplementation = randomHex(20)
      const invalidImplementation = randomHex(20)
      const externalAccount = randomHex(20)
      ...
      describe('with an invalid implementation', () => {
        it('returns an InvalidImplementation', async () => {
          const session = Session.of({})
          const walletService = await module.resolve(WalletService)
          const deployRes = await walletService.deployWallet(session, invalidImplementation, WalletProxyType.EIP1167)
      ...
      describe('with a valid implementation', () => {
        describe('on the first run', () => {
          it('sends a transaction and updates the session', async () => {
            ...
            const deployRes = await walletSvc.deployWallet(session, validImplementation, WalletProxyType.EIP1167)
            expect(deployRes.ok).toBe(true)
